...
// 各レコードに 1 から連番の id を付与
const allDataWithId: MountainRecord[] = allData.map((record, index) => ({
  id: index + 1,
  ...record,
}));

// オブジェクト配列をCSV形式の文字列に変換する関数
function convertToCSV(records: MountainRecord[]): string {
  const header = ["id", "year", "filename", "place", "title", "date", "activityType", "details"];
  ...
  for (const rec of records) {
    const row = header.map((field) => {
      let value = rec[field as keyof MountainRecord];
      if (value === null || value === undefined) {
        value = "";
      }
      if (typeof value === "string") {
        // ダブルクォートで囲み、内部のダブルクォートはエスケープ
...
// CSV形式のデータに変換
const csvData = convertToCSV(allDataWithId);

// すべての年度のデータをまとめたCSVファイルのパス
const filePath = "mountain_records_all_years.csv";

// CSVファイルを書き出す
fs.writeFileSync(filePath, csvData, { encoding: "utf8" });
console.log(`全年度のCSVファイルが作成されました: ${filePath}`);
